...
const featureLabels: Record<string, string> = {
    ...
    pool: 'مسبح خاص',
    gym: 'قاعة رياضة',
    traditional_courtyard: 'فناء تقليدي',
    terrace: 'شرفة واسعة',
    garden: 'حديقة خاصة',
    ...
    if (error || !listing) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center section-padding">
                ...
                <h2 className="text-2xl font-bold mb-4">{error || 'لم يتم العثور على الإقامة'}</h2>
                <Link href="/homes" className="btn-primary px-8">العودة إلى القائمة</Link>
            </div>
        );
    }
            ...
            <div className="pt-28">
                <div className="section-padding py-6 border-b border-gray-100 bg-white">
                    <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.2em] text-[#9CA3AF]">
                        ...
                        <Link href="/homes" className="hover:text-[#833AB4] transition-colors">الإقـامات</Link>
                        <span className="opacity-30">/</span>
                ...
                <div className="max-w-[1400px] mx-auto">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-20">
                        ...
                        <div className="lg:col-span-2 space-y-20">
                            {/* Description */}
                            <section>
                                <div className="text-[10px] font-black text-[#833AB4] uppercase tracking-[0.3em] mb-4">نـبذة عن الـمكان</div>
                            ...
                            {/* Features Grid */}
                            <section>
                                <div className="text-[10px] font-black text-[#833AB4] uppercase tracking-[0.3em] mb-4">الـخدمات والمـرافق</div>
                                <h2 className="text-3xl font-black text-[#111827] mb-10">مـا تـقدمه هـذه الـخدمة لك</h2>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    {listing.metadata?.features?.map((f: string) => (
                                        <div key={f} className="flex items-center gap-6 p-6 rounded-[24px] bg-white border border-gray-50 shadow-sm hover:shadow-md transition-shadow">
                                            <span className="text-3xl grayscale group-hover:grayscale-0 transition-all">{featureIcons[f] || '💎'}</span>
                                            <span className="text-[#111827] font-bold text-lg">{featureLabels[f] || f}</span>
                                        </div>
                                    ))}
                                </div>
                            </section>
                            ...
                            <section className="bg-[#111827] rounded-[40px] p-12 text-white relative overflow-hidden group">
                                ...
                                <div className="relative flex flex-col md:flex-row items-center gap-10">
                                    <div className="w-28 h-28 bg-gradient-to-br from-blue-600 to-indigo-900 rounded-[32px] flex items-center justify-center text-white text-5xl font-black shadow-2xl">
                                        {listing.provider?.businessName?.[0] || 'ر'}
                                    </div>
                                    <div className="flex-1 space-y-3 text-center md:text-right">
                                        <div className="inline-block px-3 py-1 bg-white/10 rounded-full text-[10px] font-black uppercase tracking-widest mb-2">مـزود خـدمة مـوثوق</div>
                                        <h3 className="text-3xl font-black">{listing.provider?.businessName}</h3>
                                        <p className="text-white/60 text-lg font-light leading-relaxed">{listing.provider?.description}</p>
                                        <div className="flex items-center justify-center md:justify-start gap-4 mt-6 pt-6 border-t border-white/5">
                                            {listing.provider?.kycStatus === 'VERIFIED' && (
                                                <span className="flex items-center gap-2 text-emerald-400 text-xs font-bold">
                                                    ...
                                                    تـم الـتحقق مـن الـهوية الـقانونية
                                                </span>
                                            )}
                                        </div>
                                    </div>
