...
export default function AddEditListingScreen() {
    ...
    const [form, setForm] = useState({
        ...
        wilaya: '',
        address: '',
        type: 'HOTEL', // Should ideally be fixed to provider's type
    });
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (isEditing) {
                ...
                Alert.alert(t('success'), t('listing_created_success'));
            }
            router.back();
        } catch (error: any) {
    ...
    return (
        <ScrollView className="flex-1 bg-white">
            <View className="bg-white px-6 pt-14 pb-4 shadow-sm flex-row items-center justify-between">
                ...
                <Text className="text-xl font-bold text-gray-900">{isEditing ? t('edit_listing') : t('create_listing')}</Text>
                <View className="w-10" />
            </View>
            ...
            <View className="px-6 py-8">
                ...
                <TouchableOpacity className="w-full h-48 bg-gray-50 rounded-3xl items-center justify-center border-2 border-dashed border-gray-200 mb-8">
                    ...
                    <Text className="text-gray-400 mt-2 font-medium">{t('add_photos') || 'Add Photos'}</Text>
                </TouchableOpacity>
                ...
                <Button
                    title={isEditing ? t('save_changes') : t('create_listing')}
